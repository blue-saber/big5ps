...
#ifdef HAVE_ICONV
#include <iconv.h>
static iconv_t				iconvp = (iconv_t)(-1);
#endif
...
static int		cmd_eofill	(char *str);
static int		cmd_setfont	(char *str);
static int		cmd_emit	(char *str);
...
static short	set_iconv_big = 1;

static int big5_to_unicode (const unsigned char *big5str) {
	wchar_t		unicode;
#ifdef HAVE_ICONV
	unsigned char	buffer[256];
#ifdef linux
	char		*inbptr = (void *) big5str;
#else
	const char	*inbptr = (void *) big5str;
#endif
	char		*outbuf = buffer;
	size_t		ileft = 2;
	size_t		oleft = sizeof buffer;
	int		i;
#endif

#ifdef HAVE_ICONV

	if (iconv (iconvp, &inbptr, &ileft, &outbuf, &oleft) < 0) {
		perror ("iconv");
		return 0;
	}
	...
	if (set_iconv_big) {
		for (i = 0; i < sizeof buffer - oleft; i++) {
			unicode <<= 8;
			unicode += buffer[i];
		}
	} else {
		for (i = sizeof buffer - oleft - 1; i >= 0; i--) {
			unicode <<= 8;
			unicode += buffer[i];
		}
	}
...
static int txtbuf_allocate (void) {
	...
	if (txtbuf_current == NULL) {
		if ((txtbuf = malloc (sizeof (
					struct textfile_buffer))) == NULL) {
			perror ("malloc");
		}
		...
		if ((ptr = malloc (sizeof (struct textfile_buffer))) == NULL) {
			perror ("malloc");
		}
...
static int push_word (char *buffer) {
	int		didx, c;
	int		advance = 0;
	unsigned char	big5str[3];
	wchar_t		unicode;
	static int	not_work = 0;

	// fprintf (stderr, "PUSH_word\n");
	...
	if ((c = get_Big5_char_index (buffer)) >= 0) {
		...
		big5str[2] = '\0';
		// mbstowcs (&unicode, big5str, 1);
		unicode = big5_to_unicode (big5str);

		if (unicode == big5str[0] * 256 + big5str[1]) {
			// seems like mbstowcs doesn't work !!
			if (! not_work) {
				not_work = 1;
				fprintf (stderr,
					"seems like mbstowcs doesn't work\n");
	...
	while (fgets (buffer, MAXLINE, fp)) {
		...
		if (linehead_cmd) {
			if (strncmp (buffer, cmdlead, cmdleadlen) == 0) {
				...
				case PCMD_EMIT:
					if (strlen (direct_str) > 0) {
						ptr = strdup (direct_str);
						tteng.push ((int) ptr,
							TBCTYPE_SET_EMIT);
					}
					break;
...
static void tte_set_fontfile (char *fontfile) {
	...
	if ((fontid = tteng.new_FontFace (fontfile)) < 0) {
		fprintf (stderr, "Can not load [%s] as font file\n",
				fontfile);
	} else {
		big5font_id = fontid;
	}
...
static int tte_new_FontFace (const char *fontfile) {
	...
	if ((ptr = malloc (sizeof (struct face_info))) == NULL) {
		...
		if ((p = strrchr (fontfile, '/')) != NULL) {
			p++;
		} else {
			p = fontfile;
		}

		ptr->fontname = strdup (p);
	}
	...
	if (layout.dynafix) {
		/* FIXME */
		ptr->xmax = (short) (((long) ptr->xmax) * 72 / em);
		ptr->ymax = (short) (((long) ptr->ymax) * 72 / em);
	} else {
		ptr->xmax = (short) (((long) ptr->xmax) * res / em);
		ptr->ymax = (short) (((long) ptr->ymax) * res / em);
	}

	// fprintf (stderr, "[%s] max(%d,%d), em=%d\n",
	//		ptr->fontname, ptr->xmax, ptr->ymax, em);

#if TT_FREETYPE_MAJOR == 1
#else
	/* FIXME */
	FT_Set_Char_Size (ptr->face, em, em, 72, 72);
#endif
...
#endif
#ifdef HAVE_ICONV
	if (iconvp >= 0) iconv_close (iconvp);
#endif
}
...
#  endif
		...
		layout.dynafix = scale;

		fprintf (stderr, "Support for DynaFont: [%s, %s]\n",
#if ENABLE_DYNA_FONT_BYTECODE == 1
				"true"
#else
				"false"
#endif
				, layout.dynafix ? "true" : "false");


#ifdef HAVE_ICONV
		//if ((iconvp = iconv_open ("UTF-8","BIG5")) == (iconv_t)(-1)) {
		if ((iconvp = iconv_open ("UCS-2", "BIG5")) == (iconv_t)(-1)) {
			perror ("iconv_open");
		} else {
			unsigned char	big5[3];

			big5[0] = 181; big5[1] = 123; big5[2] = 0;
			set_iconv_big = 0;

			fprintf (stderr, "Testing iconv: ");

			if (big5_to_unicode (big5) != 31243) {
				set_iconv_big = 1;
...
static void ttf2ps (const int fontid, const int code, const int unicode) {
...
#if ENABLE_DYNA_FONT_BYTECODE == 1
	// err = FT_Load_Glyph (ptr->face, cidx, FT_LOAD_NO_BITMAP);
#else
	// err = FT_Load_Glyph (ptr->face, cidx,
	//			FT_LOAD_NO_SCALE|FT_LOAD_NO_BITMAP);
#endif

	if (layout.dynafix) {
		// Scale is required (due to bytecode)
	...
	// FT_LOAD_FORCE_AUTOHINT, FT_LOAD_NO_RECURSE, FT_LOAD_PEDANTIC
	//
	//
	...
	if (err) {
		fprintf (stderr, "%s(%d): fail on FT_Load_Glyph\n",
			__FILE__, __LINE__);
	} else {
	...
	for (ptr = txtbuf; ptr != NULL; ptr = ptr->next) {
		...
		for (i = 0; i < upto; i++) {
			...
			case TBCTYPE_BIG5:
				...
				break;
			case TBCTYPE_SET_EMIT:
				emit = (void *) ptr->code[i].v.value;
				if (emit != NULL) {
					fprintf (psfp, "%s\n", emit);
					free (emit);
				}
