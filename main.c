...
int main (int argc, char *argv[]) {
...
#ifdef __sun__
	// putenv ("PATH_LOCALE=/usr/local/share");
	putenv ("PATH_LOCALE=/usr/lib/iconv");
#else
	setenv ("PATH_LOCALE", "/usr/share", 1);
#endif
	
	while ((c = getopt (argc, argv, "Ehwf:o:b:nt:")) != EOF) {
		switch (c) {
		case 'n':
			to_scale = 0;
			break;
	...
	fprintf (stderr,
		"big5ps %s, Copyright (c) 2002-2005,2015, written by Blue Saber.\n\n",
		VERSION);

	if (errflag) {
		fprintf (stderr, "usage: "
			"big5ps [-Enw][-b outbin][-t tray][-o outfile] file\n");
		return 1;
	}
	...
	if ((engine = Initial_TrueTypePS_Engine (psfp, lc_ctype,
						to_scale)) == NULL) {
